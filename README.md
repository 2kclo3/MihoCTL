...
- `mihoctl sub add|list|update`，其中 `sub add` 支持直接粘贴常见的一键导入链接，程序会自动解析出真实订阅地址；订阅下载会优先尝试基于兼容高版本号模板生成的 Clash Verge / FlClash / CFW / v2rayN 请求头形态，并支持 URL 中 `user:pass@host` 的 Basic Auth
- `mihoctl proxy ls|use|check`
- `mihoctl mode sys on|off`、`mihoctl mode tun on|off`、`mihoctl mode status`
  Linux 无头环境下，`mode sys` 会通过受控的 shell 环境变量脚本为新终端注入代理设置
...
```bash
./mihoctl sub use <name>
```

如果当前没有默认订阅，相关命令会直接提示你先选择，而不是自动兜底。

如果某个机场对客户端识别逻辑比较特殊，也可以直接为单个订阅覆写 User-Agent：

```bash
./mihoctl sub set-ua <name> 'clash-verge/v9.9.9'
./mihoctl sub clear-ua <name>
```
